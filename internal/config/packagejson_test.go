...
func TestLoadPackageJSON(t *testing.T) {
	cases := []struct {
		...
		{name: "routeup block capture", content: `{"routeup":{"name":"myapp","capture":{"request":true}}}`, want: Config{Name: "myapp", Capture: CaptureConfig{Request: true}}, wantHasBlock: true},
		{name: "routeup block redacted headers", content: `{"routeup":{"name":"myapp","capture":{"request":true,"redact_headers":["Authorization"]}}}`, want: Config{Name: "myapp", Capture: CaptureConfig{Request: true, RedactHeaders: []string{"Authorization"}}}, wantHasBlock: true},
