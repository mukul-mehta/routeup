...
func TestTokenHash_Deterministic(t *testing.T) {
	h1 := tokenHash("sk_routeup_example")
	if h1 != tokenHash("sk_routeup_example") {
		t.Error("tokenHash is not deterministic for the same secret")
	}
	if h1 == tokenHash("sk_routeup_wrong") {
		t.Error("tokenHash collided for different secrets")
	}
	if len(h1) != 64 { // hex-encoded SHA-256 is 32 bytes -> 64 hex chars
		t.Errorf("tokenHash len = %d, want 64", len(h1))
	}
}
