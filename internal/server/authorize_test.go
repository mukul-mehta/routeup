...
func TestAuthorize_TokenAccept(t *testing.T) {
	...
	if d.Mode != ClaimToken || d.Ephemeral {
...
func TestAuthorize_NamespaceAccept(t *testing.T) {
	...
	if d.Host != "foo.try.routeup.dev" || d.Mode != ClaimNamespace || !d.Ephemeral {
...
func TestAuthorize_TokenRandom(t *testing.T) {
	az, _, secret := authzFixture(t, "try")
	d, err := az.Authorize(context.Background(), ClaimAttempt{TokenSecret: secret, Random: true})
	if err != nil {
		t.Fatalf("Authorize random: %v", err)
	}
	if !strings.HasSuffix(d.Host, ".alice.routeup.dev") {
		t.Errorf("random host = %q, want suffix .alice.routeup.dev", d.Host)
	}
}
