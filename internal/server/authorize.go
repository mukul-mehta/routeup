...
// HoldRequest returns the request the caller passes to Store.HoldRoute to hold
// the decided host.
func (d Decision) HoldRequest() HoldRequest { return d.holdReq }

// ClaimAttempt is a request to claim a public route. An empty TokenSecret takes
// the public-namespace (try) path.
...
func (a *Authorizer) authorizeToken(ctx context.Context, attempt ClaimAttempt) (Decision, error) {
	...
	req := HoldRequest{Host: host, Kind: HoldByToken, TokenID: tok.ID}
	return Decision{Host: host, Mode: HoldByToken, Base: base, holdReq: req}, nil
}
...
func (a *Authorizer) authorizeNamespace(ctx context.Context, attempt ClaimAttempt) (Decision, error) {
	...
	req := HoldRequest{Host: host, Kind: HoldByNamespace}
	return Decision{Host: host, Mode: HoldByNamespace, Ephemeral: true, Base: base, holdReq: req}, nil
}
...
// host is one label under its base, so multi-label route names are rejected.
