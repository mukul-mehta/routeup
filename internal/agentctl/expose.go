...
// Expose sends POST /v1/expose and returns the granted public host. Establishing
// a tunnel can take longer than the default client timeout, so this uses a
// dedicated client bounded only by ctx (and a generous ceiling).
func (c *Client) Expose(ctx context.Context, req ipc.ExposeRequest) (ipc.ExposeResponse, error) {
	...
	longClient := &http.Client{Transport: c.httpClient.Transport, Timeout: 60 * time.Second}
